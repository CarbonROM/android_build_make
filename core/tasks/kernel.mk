...
ifeq ($(TARGET_KERNEL_CLANG_COMPILE),true)
    ...
    KERNEL_CLANG_VERSION := $(find $(ANDROID_BUILD_TOP)/prebuilts/clang/host/$(HOST_OS)-x86/ -name AndroidVersion.txt -exec grep -l $(TARGET_KERNEL_CLANG_VERSION) "{}" \; | sed -e 's|/AndroidVersion.txt$||g;s|^.*/||g')
...
TARGET_KERNEL_BINARIES: $(KERNEL_OUT_STAMP) $(KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL_STAMP)
	...
	$(hide) if [ ! -z "$(TARGET_NEEDS_DTBOIMAGE)" ]; then \
			...
			$(MAKE) $(MAKE_FLAGS) -C $(KERNEL_SRC) O=$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) $(KERNEL_CROSS_COMPILE) $(KERNEL_CLANG_TRIPLE) $(KERNEL_CC) dtbo.img; \
		fi
...
dtboimage: $(KERNEL_OUT_STAMP) $(KERNEL_CONFIG)
	echo -e ${CL_GRN}"Building DTBO.img"${CL_RST} ; \
	$(MAKE) $(MAKE_FLAGS) -C $(KERNEL_SRC) O=$(KERNEL_OUT) ARCH=$(KERNEL_ARCH) $(KERNEL_CROSS_COMPILE) $(KERNEL_CLANG_TRIPLE) $(KERNEL_CC) dtbo.img; \
